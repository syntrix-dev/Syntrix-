...
export default function Analysis({ analysis, loading, error }) {
  ...
  if (error?.analysis || !analysis) {
    return (
      <div style={{ color: '#ef4444', fontSize: 14, padding: 12,
        ...
        ✗ Failed to load analysis: {error?.analysis?.message || 'Unknown error'}
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        <StatCard label="Issues Found" value={analysis.issues_found}
          sub="wasteful resources" accent="#ef4444" />
        <StatCard label="Savings / Month"
          value={`$${fmt(analysis.potential_savings_usd_per_month)}`}
          sub={`₹${fmt(savings, 0)}/mo`} accent="#00c4a4" />
        <StatCard label="Syntrix Fee (20%)"
          value={`₹${fmt(fee, 0)}`} sub="charged only on actual savings" />
        <StatCard label="Your Net Gain"
          value={`₹${fmt(netMonthly, 0)}`} sub="monthly net savings" accent="#1a7aff" />
      </div>

      <Card>
        <SectionLabel>Detected Issues</SectionLabel>
        {analysis.issues.length === 0 ? (
          <EmptyState icon="✓" title="No waste detected"
                ...
                <div key={i} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: '#8899bb', marginTop: 2 }}>
                      {issue.type === 'idle_ec2'
                        ? `${issue.instance_type || ''} · CPU ${issue.cpu_percent ?? ''}% avg`
                        : 'Hidden or underutilised service cost'}
                    </div>
                  </div>
      ...
      <Card accent="#00c4a4">
        ...
        <div style={{ marginTop: 16, padding: '12px 16px',
          background: 'rgba(0,196,164,0.05)', borderRadius: 9,
          fontSize: 12, color: '#8899bb', lineHeight: 1.6 }}>
          <strong style={{ color: '#00c4a4' }}>Pricing model:</strong> Syntrix charges 20% of verified monthly savings only.
          You pay ₹{fmt(fee, 0)}/mo and keep ₹{fmt(netMonthly, 0)}/mo — no upfront costs, no savings = no fee.
        </div>
      </Card>
